...
where @URL@ is the URL of the plugin from the compatibility table. For examples to install version @5.0.2@ run

<pre>
bin/elasticsearch-plugin install http://dl.bintray.com/content/imotov/elasticsearch-plugins/org/elasticsearch/elasticsearch-analysis-morphology/5.0.2/elasticsearch-analysis-morphology-5.0.2.zip
</pre>
...
where @URL@ is the URL of the plugin from the compatibility table. For examples to install version @2.4.2@ run
